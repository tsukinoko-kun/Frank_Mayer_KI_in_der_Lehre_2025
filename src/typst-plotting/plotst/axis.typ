...
#let axis(
  ...
  show_markings: true,
  invert_markings: false,
  marking_offset_left: 1,
  marking_offset_right: 0,
  stroke: black,
  marking_color: black,
  value_color: black,
  helper_lines: false,
  helper_line_style: "dotted",
  helper_line_color: gray,
  marking_length: 5pt,
  marking_number_distance: 5pt,
  title: [],
  value_formatter: i => i,
) = {
  // TODO automate? macro-programming?
  let axis_data = (
    min: min,
    max: max,
    step: step,
...
#let measure_axis(axis, style) = {
  ...
  let dist = if axis.invert_markings {
    axis.marking_length + axis.marking_number_distance
  } else { axis.marking_number_distance }
  let inversion = if axis.invert_markings == -1 { dist * 2 + size.width } else {
    0pt
  }
...
#let draw_axis(axis, length: 100%, pos: (0pt, 0pt)) = {
  ...
  if is_vertical(axis) {
    ...
    place(
      dx: pos.at(0),
      dy: pos.at(1),
      line(angle: -90deg, length: length - arrow_size * 2, stroke: axis.stroke),
    )
    ...
    style(style => {
      ...
      if axis.location == "left" {
        place(
          dy: pos.at(1) - length / 2,
          dx: -length / 2 - a,
          rotate(
            -90deg,
            origin: center + top,
            box(width: length, height: 0pt, align(center + top, axis.title)),
          ),
        )
      } else {
        place(
          ...
          rotate(
            -90deg,
            origin: center + top,
            box(width: length, height: 0pt, align(center + bottom, axis.title)),
          ),
        )
      }
    })
    ...
    // Draws step markings
    for step in range(
      axis.marking_offset_left,
      axis.values.len() - axis.marking_offset_right,
    ) {
      // Draw helper lines:
      ...
      if axis.show_markings {
        place(
          dx: pos.at(0),
          dy: pos.at(1) - step_length * step,
          line(
            angle: 0deg,
            length: axis.marking_length
              * invert_markings
              * user_invert_markings,
            stroke: axis.marking_color,
          ),
        )
      }
      // Draw numbering
      if axis.show_values {
        ...
        style(styles => {
          ...
          let inversion = if invert_markings == -1 {
            dist * 2 + size.width
          } else { 0pt }
          place(
            dx: pos.at(0) - dist - size.width + inversion,
            dy: pos.at(1) - step_length * step - 4pt,
            text(fill: axis.value_color, number),
          )
        })
      }
    }
    ...
    place(
      dx: pos.at(0),
      dy: pos.at(1),
      line(angle: 0deg, length: length - arrow_size * 2, stroke: axis.stroke),
    )

    // draw the arrow at the end of the axis
    if axis.show_arrows {
      place(
        dx: pos.at(0) + length - arrow_size * 2,
        dy: pos.at(1),
        polygon(
          fill: axis.stroke,
          (0pt, -arrow_size),
    ...
    if axis.location == "bottom" {
      place(
        dx: pos.at(0),
        dy: 3pt,
        align(top + center, box(width: length, height: 0pt, [\ #axis.title])),
      )
    } else {
      style(style => {
    ...
    for step in range(
      ...
      if axis.show_values {
        ...
        style(styles => {
          let size = measure([#number], styles)
          let dist = if axis.invert_markings {
            axis.marking_number_distance + axis.marking_length
          } else { axis.marking_number_distance }
          let inversion = if invert_markings == -1 {
            -dist * 2 - size.height
          } else { 0pt }
          place(
            dx: pos.at(0) + step_length * step,
            dy: pos.at(1) + dist + inversion,
            box(
              width: 0pt,
              align(center, text(fill: axis.value_color, str(number))),
            ),
          )
        })
      }
    ...
    for step in range(
      ...
      if axis.helper_lines {
        place(
          ...
          line(
            angle: 0deg,
            length: length * invert_markings,
            stroke: (
              paint: axis.helper_line_color,
              dash: axis.helper_line_style,
            ),
          ),
        )
      }
