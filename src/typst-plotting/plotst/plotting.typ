          ...
          for (idx, plot) in plots.enumerate() {
            if idx == 0 {
              place(
                dx: 0pt,
                dy: 0pt,
                box(width: 100%, height: 100%, plot.body.child.body),
              )
            } else {
              place(
                dx: 0pt,
                ...
                box(
                  width: 100%,
                  height: 100%,
                  plot.body.child.body.children.at(1),
                ),
              )
            }
          }
  ...
  let plot_code() = {
    ...
    for (x, y) in plot.data {
      ...
      if stroke != none or fill != none {
        // DELETEME deprecation, only keep else
        draw_marking(
          ((x - x_axis.min) * step_size_x, -(y - y_axis.min) * step_size_y),
          square(width: 2pt, height: 2pt, fill: fill, stroke: stroke),
        )
      } else {
        draw_marking(
          ((x - x_axis.min) * step_size_x, -(y - y_axis.min) * step_size_y),
...
#let graph_plot(
  ...
  let plot_code() = {
    ...
    for i in range(data.len()) {
      ...
      if i == 0 {
        delta.push((
          rounding * (next.at(0) - curr.at(0)),
          rounding * (next.at(1) - curr.at(1)),
        ))
      } else if i == data.len() - 1 {
        delta.push((
          rounding * (curr.at(0) - prev.at(0)),
          rounding * (curr.at(1) - prev.at(1)),
        ))
      } else {
        delta.push((
          rounding * .5 * (next.at(0) - prev.at(0)),
          rounding * .5 * (next.at(1) - prev.at(1)),
        ))
      }
    }
...
#let histogram(
  ...
  let plot_code() = {
    ...
    for value in bin_count {
      common_class = if value.at(0) > common_class.at(0) { value } else {
        common_class
      }
    }
  ...
  let plot_code() = {
    ...
    layout(size => {
      ...
      let radius = (
        min(
          size.width,
          if display_style.split("-").at(0) == "vert" {
            size.height - 30pt
          } else { size.height },
        )
          / 2
      ...
      for (i, data) in data.enumerate() {
        ...
        pie += place(
          dy: -radius,
          dx: dx,
          path(fill: colors.at(calc.rem(i, colors.len())), ..points),
        )

        if display_style == "legend-inside-chart" {
          let pnt = point(angle + fraction / 2)
          pie += place(
            dx: pnt.at(0) * radius * offset + dx,
            dy: pnt.at(1) * radius * offset - radius,
            box(
              width: 0pt,
              height: 0pt,
              align(center + horizon, box(fill: purple, str(data.at(1)))),
            ),
          )
        } else {
          legend += (
            text(fill: colors.at(calc.rem(i, colors.len())), sym.hexa.filled)
              + sym.space.thin
              + str(data.at(1))
          )
          if i != plot.data.len() - 1 {
            legend += if "hor" in display_style { "\n" } else { sym.space.quad }
          }
        }
        angle += fraction
      }
      style(s => {
        ...
        if display_style == "vert-chart-legend" {
          ...
          place(
            dx: 50% + radius - legend-size.width / 2 + 10pt,
            dy: -radius,
            box(height: 0pt, align(horizon, legend)),
          )
  ...
  let plot_code() = {
    ...
    if not rotated {
      for (idx, data_set) in data.enumerate() {
        ...
        let x_pos = (
          x_data * step_size_x
            - if centered_bars { step_size_x * bar_width / 2 } else { 0pt }
        )
        place(
          dx: x_pos,
          dy: -height,
          rect(
            width: step_size_x * bar_width,
            height: height,
            fill: if array_fill { fill.at(idx) } else { fill },
            stroke: if array_stroke { stroke.at(idx) } else { stroke },
          ),
      ...
      for (idx, data_set) in data.enumerate() {
        ...
        let y_pos = (
          y_data * step_size_y
            + if centered_bars { step_size_y * bar_width / 2 } else { 0pt }
        )
        place(
          dx: 0pt,
          dy: -y_pos,
        ...
        width: scaling,
        {
          layout(size => {
            ...
            place({
              ...
              let translate(x, y) = {
                (
                  calc.sin(360deg / x_size * x) * y * step_size,
                  -calc.cos(360deg / x_size * x) * y * step_size,
                )
              }
                  ...
                  place(
                    ...
                    box(
                      ...
                      align(
                        center + horizon,
                        text(
                          str(x_axis.values.at(i)),
                          fill: x_axis.value_color,
                        ),
                      ),
                    ),
                  )
                ...
                for x in range(x_size) {
                  ...
                  if (y_axis.show_markings) {
                    place(
                      line(
                        start: points.at(-1),
                        angle: 360deg / x_size * x,
                        length: y_axis.marking_length,
                      ),
                    )
                    place(
                      line(
                ...
                if (y_axis.helper_lines) {
                  place(
                    path(
                      ...
                      closed: true,
                      stroke: (
                        paint: y_axis.helper_line_color,
                        dash: y_axis.helper_line_style,
                      ),
                    ),
                  )
                }
  ...
  let plot_code() = {
    ...
    let data = calc_data.map(dataset => (
      ...
      if calc.rem(dataset.len() * 0.25, 1) != 0 {
        ...
        (
          (
            dataset.at(int(dataset.len() * 0.25) - 1)
              + dataset.at(int(dataset.len() * 0.25))
          )
            / 2
        )
      ...
      if calc.rem(dataset.len() * 0.25, 1) != 0 {
        ...
        (
          (
            dataset.at(int(dataset.len() * 0.5) - 1)
              + dataset.at(int(dataset.len() * 0.5))
          )
            / 2
        )
      ...
      if calc.rem(dataset.len() * 0.25, 1) != 0 {
        ...
        (
          (
            dataset.at(int(dataset.len() * 0.75) - 1)
              + dataset.at(int(dataset.len() * 0.75))
          )
            / 2
        )
    ...
    for (idx, data_set) in data.enumerate() {
      ...
      place(
        dx: x_pos + (box_width - whisk_width) * .5,
        dy: -q(0),
        line(length: whisk_width),
      )
      place(
        dx: x_pos + box_width * .5,
        dy: -q(0),
        line(end: (0pt, q(0) - q(1)), stroke: whisker_stroke),
      )
      place(
        dx: x_pos + (box_width - whisk_width) * .5,
        dy: -q(4),
      ...
      place(
        dx: x_pos + box_width * .5,
        dy: -q(3),
        line(end: (0pt, q(3) - q(4)), stroke: whisker_stroke),
      )
    }
  }
  prepare_plot(size, caption, plot_code, plot: plot, render_axis: render_axes)
